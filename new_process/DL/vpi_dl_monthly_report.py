...
Usage:
    python vpi_dl_monthly_report.py                    # uses first day of previous month
    python vpi_dl_monthly_report.py 2026-05-01         # explicit snapshot date
Output:
    vpi_dl_report_May_2026.html  (in same directory as this script)
"""
...
WITH prev AS (
   ...
   FROM {REF}
   WHERE cptmonth=%s AND {TRGPRJD}='y' AND projecteddate BETWEEN 2026 AND 2030
   GROUP BY 1,2,3
),
curr AS (
   SELECT projecteddate, bandgrp, vendor,
...
SELECT COUNT(*) AS total_rows,
       ...
       SUM(CASE WHEN {TRGPRJD}   IS NULL THEN 1 ELSE 0 END) AS null_trgprjd,
...
if DATA_MODE == 'csv':
    ...
    for key, (sql, params) in QUERIES.items():
        ...
        results[key] = execute_query_on_csv(key, updated_params, csv_files, STAGE, REF, TRGPRJD, TRGCURR, h0, h1)
...
def market_movers_html():
    ...
    for old_mkt, new_mkt in sorted(rehome_src.items()):
        ...
        rehome_drop_html += (
            ...
            f'<span class="text-red">{vstr}</span> ({fmt(m1)} &#8594; {fmt(c)})</li>'
        )
...
def render_dq_table():
    ...
    rows_data = [
        ...
        (f'Null {TRGPRJD}',         fmt(r.get('null_trgprjd')),      r.get('null_trgprjd')),
        ('Null cec_curr',           fmt(r.get('null_cec_curr')),     r.get('null_cec_curr')),
        ('Null cec_prjd',           fmt(r.get('null_cec_prjd')),     r.get('null_cec_prjd')),
    ]
...
n_market_band_flags = market_band_flags_count()
...
# Create a more concise rehome summary
if rehome_pairs:
    # Count unique rehome mappings
    unique_mappings = len(set((r['old_market'], r['new_market']) for r in rehome_pairs))
    # Show first few mappings and count if there are many
    if len(rehome_pairs) <= 5:
        mappings_str = ' | '.join(f"Mkt {r['old_market']} → Mkt {r['new_market']}" for r in rehome_pairs)
    else:
        first_few = ' | '.join(f"Mkt {r['old_market']} → Mkt {r['new_market']}" for r in rehome_pairs[:3])
        mappings_str = f"{first_few} | ... ({len(rehome_pairs)} total mappings)"
    
    rehome_summary_box = (
        f'<div class="insight good"><b>Confirmed Rehome ({unique_mappings} unique mappings):</b> '
        f'{mappings_str} &mdash; volume drop in these markets is expected.</div>'
    )
else:
    rehome_summary_box = ''
...
lost_by_band_insight = (
    f'<div class="insight crit"><b>{top_band_name} dominates the loss</b> — '
    f'{top_band_pct}%% of lost carriers from unexplained markets are {top_band_name} '
    f'({fmt(top_band_cnt)} carriers). Investigate upstream pipeline/ingestion for {h0}.</div>'
...
<title>VPI DL Staging Analysis — {h0}</title>
<style>{CSS}</style>
</head>
<body>
<div class="page">
...
<div class="header">
  <h1>VPI DL Staging Analysis — {h0}</h1>
...
<div class="section">
  <div class="section-title">A2 — Market Volume: 3-Month Trend
    ...
    <span class="badge badge-ok">{len(unique_rehome_src)} Confirmed Rehome</span>
  </div>
...
<!-- A3 -->
<div class="section">
  <div class="section-title">A3 — {TRGPRJD}='y' Band/Vendor by Projected Year (2026–2030)
    <span class="badge badge-crit">{len(bvf)} Combo(s) Flagged Across All Years</span>
  </div>
  <div class="section-desc">Carriers with DL projected target flag, {h1} ref &#8594; {h0} stage, all projection years (2026–2030).</div>
...
<!-- A7 -->
<div class="section">
  <div class="section-title">A7 — {TRGPRJD} Flag Distribution by Projected Year ({h0} Stage)
    <span class="badge badge-ok">Stable</span>
  </div>
  <div class="section-desc">How {TRGPRJD} ('y'/'n') distributes across each projected year in the {h0} staging table.</div>
...
<!-- A11 -->
<div class="section">
  <div class="section-title">A11 — {TRGCURR} Distribution Per Market, Per Band Group ({h0} Stage)
...
<div class="section">
  ...
  {'<div class="insight crit"><b>Action Required — ' + ', '.join(f"{bg}/{v}" for bg,v,_ in bvf) + f' ({TRGPRJD}=y):</b> These band/vendor combinations declined consistently across all projection years (2026–2030). Investigate pipeline/classification before publishing.</div>' if bvf else ''}
  {rehome_summary_box}
  <div class="insight {'crit' if n_unexplained > 0 else 'warn'}">
    ...
    {f'<b>{top_band_pct}%% of the lost carriers are {top_band_name}</b> ({fmt(top_band_cnt)} carriers) — investigate {top_band_name} pipeline/ingestion for {h0}.' if top_band_row else ''}
